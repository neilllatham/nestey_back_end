...
\restrict 1rsfV27oM2e6o0teVbFdlv0yMJ25rioEqp1n5MIxOG4GhHAYqNQj8IibB5dg7we
...
CREATE TABLE public.employee_reviews (
    ...
    review_phase character varying DEFAULT 'build-plan'::character varying,
    mid_year_completed_at timestamp without time zone,
    end_year_completed_at timestamp without time zone,
    ...
    CONSTRAINT review_phase_check CHECK (((review_phase)::text = ANY ((ARRAY['build-plan'::character varying, 'mid-year'::character varying, 'end-of-year'::character varying])::text[])))
);
...
--
-- Name: employee_301_export; Type: VIEW; Schema: public; Owner: neill
--

CREATE VIEW public.employee_301_export AS
 SELECT e.employee_id,
    ...
    e.bonus_eligible,
    e.target_bonus,
    e.commission_eligible,
    e.commission_target,
    e.compensation_currency,
    e.date_of_birth,
    e.employment_type,
    e.gender,
    e.home_address,
    ...
    jf.job_family_name,
    rl.role_level_name,
    rl.description AS role_level_description,
    m.employee_id AS manager_id,
    m.full_name AS manager_name,
    m.job_title AS manager_title,
    ( SELECT json_agg(row_to_json(a.*)) AS json_agg
           FROM public.time_off_accruals a
          WHERE (a.employee_id = e.employee_id)) AS time_off_accruals,
    ( SELECT json_agg(row_to_json(rq.*)) AS json_agg
           FROM public.time_off_requests rq
          WHERE (rq.employee_id = e.employee_id)) AS time_off_requests,
    ( SELECT json_agg(json_build_object('employee_benefit', row_to_json(eb.*), 'benefit_plan', row_to_json(bp.*), 'benefits_catalog', row_to_json(bc.*))) AS json_agg
           FROM ((public.employee_benefits eb
             LEFT JOIN public.benefit_plan bp ON ((bp.benefit_plan_id = eb.benefit_plan_id)))
             LEFT JOIN public.benefits_catalog bc ON ((bc.benefit_plan_id = bp.benefit_plan_id)))
          WHERE (eb.employee_id = e.employee_id)) AS benefits,
    ( SELECT json_agg(json_build_object('review', row_to_json(er.*), 'review_cycle', row_to_json(rc.*), 'goals', ( SELECT json_agg(json_build_object('goal', row_to_json(g.*), 'strategy', row_to_json(s.*))) AS json_agg
                   FROM (public.employee_goals g
                     LEFT JOIN public.strategies s ON ((s.strategy_id = g.strategy_id)))
                  WHERE (g.review_id = er.review_id)), 'competencies', ( SELECT json_agg(json_build_object('competency', row_to_json(c.*), 'employee_competency', row_to_json(ec.*))) AS json_agg
                   FROM (public.employee_competencies ec
                     LEFT JOIN public.core_competencies c ON ((ec.competency_id = c.competency_id)))
                  WHERE (ec.review_id = er.review_id)))) AS json_agg
           FROM (public.employee_reviews er
             LEFT JOIN public.review_cycles rc ON ((rc.review_cycle_id = er.review_cycle_id)))
          WHERE (er.employee_id = e.employee_id)) AS performance
   FROM ((((((public.employees e
     LEFT JOIN public.departments d ON ((d.department_id = e.department_id)))
     LEFT JOIN public.functions f ON ((f.function_id = d.function_id)))
     LEFT JOIN public.roles r ON ((r.role_id = e.role_id)))
     LEFT JOIN public.job_families jf ON ((jf.job_family_id = r.job_family_id)))
     LEFT JOIN public.role_levels rl ON (((rl.role_level_code)::text = (r.role_level_code)::text)))
     LEFT JOIN public.employees m ON ((m.employee_id = e.manager_id)))
  WHERE (e.employee_id = 301);


ALTER TABLE public.employee_301_export OWNER TO neill;
...
COPY public.employee_goals (goal_id, review_id, strategy_id, objective, goal_description, target_date, employee_rating, manager_rating, created_at, updated_at, status) FROM stdin;
8	2	2	Strengthen team collaboration and retention	Implement peer mentoring and support onboarding for 2 new team members	2025-09-30	\N	\N	2025-11-07 14:50:47.119711	2025-11-07 14:50:47.119711	active
9	2	3	Contribute to operational efficiency	Automate weekly report process to reduce manual effort by 20%	2025-12-31	\N	\N	2025-11-07 14:50:47.119711	2025-11-07 14:50:47.119711	active
13	3	1	Transform the customer experience	Deliver a unified customer experience strategy across all business units, targeting a 20% improvement in Net Promoter Score (NPS) and faster digital adoption.	2025-12-31	\N	\N	2025-11-07 14:59:47.807375	2025-11-07 14:59:47.807375	active
14	3	2	Elevate employee engagement and culture	Achieve top-quartile employee engagement by launching the "One Company" culture initiative and increasing retention among top performers by 15%.	2025-12-31	\N	\N	2025-11-07 14:59:47.807375	2025-11-07 14:59:47.807375	active
15	3	3	Enhance enterprise value through innovation and efficiency	Drive 10% EBITDA growth and 12% revenue growth by scaling automation, optimizing cost structure, and expanding into two new market segments.	2025-12-31	\N	\N	2025-11-07 14:59:47.807375	2025-11-07 14:59:47.807375	active
7	2	1	Support customer experience improvement	Improve customer satisfaction scores through enhanced service delivery and proactive communication	2025-12-31	\N	\N	2025-11-07 14:50:47.119711	2025-11-15 03:18:06.44	active
5	1	2	Improve employee engagement	Implement quarterly feedback surveys and increase employee engagement by 10%. Increase retention of top performers by 20%	2025-12-31	3	4	2025-11-07 14:44:51.112438	2025-11-16 07:37:45.762	active
16	1	3	Increase revenue by expanding into new small business segment.	Launch existing mid market products into small business sector to expand into new market segments. Add 5% revenue growth as a result of market expansion.	2025-12-31	4	4	2025-11-15 15:11:25.817025	2025-11-16 18:47:57.493	active
6	1	3	Drive operational efficiency	Reduce operating cost per transaction to $5.00 through automation initiatives. Improving overall margin by 15%.	2025-12-31	3	3	2025-11-07 14:44:51.112438	2025-11-17 18:38:44.077	active
4	1	1	Enhance customer experience	Achieve a 40% increase in Net Promotor Score (NPS) by end of FY26	2025-12-30	2	2	2025-11-07 14:44:51.112438	2025-11-18 20:48:48.452	active
\.
...
COPY public.employee_reviews (review_id, employee_id, review_cycle_id, created_at, updated_at, review_status, submitted_at, finalized_at, employee_overall_comments, manager_overall_comments, employee_overall_rating, manager_overall_rating, review_phase, mid_year_completed_at, end_year_completed_at) FROM stdin;
3	1	1	2025-11-07 14:54:57.912721	2025-11-07 14:54:57.912721	Draft	\N	\N	\N	\N	\N	\N	build-plan	\N	\N
2	107	1	2025-11-07 14:50:47.116862	2025-11-07 14:50:47.116862	Draft	\N	\N	Updated schema test comments	Manager updated test	5	4	build-plan	\N	\N
4	2	1	2025-11-14 19:38:23.916901	2025-11-14 19:38:23.916901	Draft	\N	\N	\N	\N	\N	\N	build-plan	\N	\N
1	301	1	2025-11-07 14:43:08.336848	2025-11-16 23:15:02.493	Draft	\N	\N	This year, I led my team to make strong progress across customer, people, and revenue objectives, including improving customer-experience foundations and helping expand into the small-business segment. We also advanced employee-engagement efforts and delivered automation that reduced costs and improved margins. The team’s work shows clear momentum and strong impact.	You showed strong leadership this year and guided your team to real progress, especially in customer experience, employee engagement, and market expansion. Your ability to collaborate and move initiatives forward stood out. To elevate your impact further, there is an opportunity to deepen and scale operational-efficiency improvements across more teams and processes.	4	4	build-plan	\N	\N
\.
...
335	2	Sick Leave	2025	5.0	0.0	5.0	2025-11-25
2	10	PTO	2025	11.6	0.0	11.6	2025-11-02
4	2	PTO	2025	17.0	7.0	10.0	2025-12-02
...
21	2	PTO	2025-11-26	2025-11-28	3.0	Pending	2025-11-25 07:51:06.711604	2025-11-25 07:51:06.711604
22	2	PTO	2025-12-08	2025-12-09	2.0	Pending	2025-11-29 13:21:57.006209	2025-11-29 13:21:57.006209
23	2	PTO	2025-12-11	2025-12-12	2.0	Pending	2025-12-02 12:40:20.963572	2025-12-02 12:40:20.963572
\.
...
SELECT pg_catalog.setval('public.employee_goals_goal_id_seq', 18, true);
...
SELECT pg_catalog.setval('public.employee_reviews_review_id_seq', 4, true);
...
SELECT pg_catalog.setval('public.time_off_requests_request_id_seq', 23, true);
...
--
-- Name: employee_reviews employee_reviews_employee_overall_rating_fkey; Type: FK CONSTRAINT; Schema: public; Owner: neill
--

ALTER TABLE ONLY public.employee_reviews
    ADD CONSTRAINT employee_reviews_employee_overall_rating_fkey FOREIGN KEY (employee_overall_rating) REFERENCES public.rating_scale(rating_value);


--
-- Name: employee_reviews employee_reviews_manager_overall_rating_fkey; Type: FK CONSTRAINT; Schema: public; Owner: neill
--

ALTER TABLE ONLY public.employee_reviews
    ADD CONSTRAINT employee_reviews_manager_overall_rating_fkey FOREIGN KEY (manager_overall_rating) REFERENCES public.rating_scale(rating_value);
...
\unrestrict 1rsfV27oM2e6o0teVbFdlv0yMJ25rioEqp1n5MIxOG4GhHAYqNQj8IibB5dg7we
